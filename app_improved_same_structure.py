"""
CHP Steenwijk — Improved Plant Performance App (same tab structure as original)
Run with:
...
from thermo_engine_improved import analyse_cycle, compute_scenario, SCENARIO_DEFS

st.set_page_config(page_title="CHP Steenwijk — Improved Plant Performance", layout="wide", page_icon="🔥")
...
# ═══════════════════════════════════════════════
with tabs[0]:
    st.title("CHP Steenwijk — Improved Plant Performance")
...
with tabs[-1]:
    ...
    This analysis uses a **Rankine-cycle representation** of the extraction-condensing turbine train,
    with steam properties from **CoolProp / IAPWS-IF97**.
    It keeps the original app structure, but improves the plant model in four places:
    1. **Useful heat** in expander cases uses the **post-expander enthalpy**
    2. **Net export** is calculated as **gross power minus auxiliary loads**
    3. **HP / LP / expander efficiencies** are corrected for **part-load** operation
    4. **S4** is treated as a **summer backpressure** case, not just a lower-availability case
    """)
