...
def _phase_from_PH(P_bar: float, h_kJkg: float) -> tuple[Optional[float], str, float, float]:
    P_Pa = P_bar * 1e5
    h_f = CP.PropsSI("H", "P", P_Pa, "Q", 0, "Water") / 1000
    h_g = CP.PropsSI("H", "P", P_Pa, "Q", 1, "Water") / 1000
    x = None
    phase = "superheated"
    if h_f <= h_kJkg <= h_g:
        x = (h_kJkg - h_f) / (h_g - h_f)
        phase = f"two-phase (x={x:.3f})"
    return x, phase, h_f, h_g


def compute_state_from_PT(name: str, P_bar: float, T_C: float) -> SteamState:
    P_Pa = P_bar * 1e5
    T_K = T_C + 273.15
...
def compute_state_from_PS(name: str, P_bar: float, s_kJkgK: float) -> SteamState:
    P_Pa = P_bar * 1e5
    s_SI = s_kJkgK * 1000
    h = CP.PropsSI("H", "P", P_Pa, "S", s_SI, "Water") / 1000
    T = CP.PropsSI("T", "P", P_Pa, "S", s_SI, "Water") - 273.15
    x, phase, _, _ = _phase_from_PH(P_bar, h)
    if x is not None:
        T = CP.PropsSI("T", "P", P_Pa, "Q", 0, "Water") - 273.15
    return SteamState(name=name, P_bar=P_bar, T_C=T, h_kJkg=h, s_kJkgK=s_kJkgK, x=x, phase=phase)


def compute_state_from_PH(name: str, P_bar: float, h_kJkg: float) -> SteamState:
    P_Pa = P_bar * 1e5
    h_SI = h_kJkg * 1000
    T = CP.PropsSI("T", "P", P_Pa, "H", h_SI, "Water") - 273.15
    s = CP.PropsSI("S", "P", P_Pa, "H", h_SI, "Water") / 1000
    x, phase, _, _ = _phase_from_PH(P_bar, h_kJkg)
    if x is not None:
        T = CP.PropsSI("T", "P", P_Pa, "Q", 0, "Water") - 273.15
    return SteamState(name=name, P_bar=P_bar, T_C=T, h_kJkg=h_kJkg, s_kJkgK=s, x=x, phase=phase)
...
def analyse_cycle(
    ...
    s1 = compute_state_from_PT("1_LiveSteam", P_live_bar, T_live_C)
    s2s = compute_state_from_PS("2s_HP_Isentropic", P_extraction_bar, s1.s_kJkgK)
    h2 = s1.h_kJkg - eta_HP * (s1.h_kJkg - s2s.h_kJkg)
    s2 = compute_state_from_PH("2_HP_Actual", P_extraction_bar, h2)

    s3s = compute_state_from_PS("3s_LP_Isentropic", P_condenser_bar, s2.s_kJkgK)
    h3 = s2.h_kJkg - eta_LP * (s2.h_kJkg - s3s.h_kJkg)
    s3 = compute_state_from_PH("3_LP_Actual", P_condenser_bar, h3)

    h3e = s2.h_kJkg - eta_exp * (s2.h_kJkg - s3s.h_kJkg)
    s3e = compute_state_from_PH("3e_Expander", P_condenser_bar, h3e)

    s4 = compute_state_from_PT("4_Feedwater", P_live_bar, T_feedwater_C)
    ...
    return CycleResult(
        state1=s1, state2s=s2s, state2=s2, state3s=s3s, state3=s3,
        state3e=s3e, state4=s4,
...
def part_load_corrected_eta(base_eta: float, load_ratio: float, section: str) -> float:
    """
    Simple transparent correction for off-design turbine efficiency.
    - HP: moderate penalty below design steam rate
    - LP: slightly stronger penalty because the LP section is more sensitive to low flow/wetness
    - EXP: mild penalty for screw expander off-design flow
    """
...
def estimate_auxiliary_load_kW(
    ...
    """
    Site-informed, transparent auxiliary-load estimate.
    Calibrated so the S1 baseline net export is close to the measured ~1.32 MW order of magnitude.
    """
    base_misc_kW = 115.0  # controls, pumps, lighting, instrumentation, water treatment, etc.
...
SCENARIO_DEFS: List[Dict[str, Any]] = [
    ...
    dict(
        ...
        condenser_pressure_bar=0.28,  # summer backpressure case
...
if __name__ == "__main__":
    ...
    print("=" * 88)
    print("CHP STEENWIJK — IMPROVED PLANT PERFORMANCE MODEL")
    print("=" * 88)
    for r in results:
        print(
    ...
    with open("/mnt/data/thermo_engine_improved_results.json", "w", encoding="utf-8") as f:
        json.dump(payload, f, indent=2)
    print("Wrote /mnt/data/thermo_engine_improved_results.json")
